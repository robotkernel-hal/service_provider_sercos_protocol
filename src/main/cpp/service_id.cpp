...
using namespace service_provider::sercos_protocol;
...
string service_id::data_to_string(const std::vector<uint16_t>& val) {
    ...
    switch (data.attr.datalength) {
        ...
        case SSA_DATALENGTH_4BYTEFIX:
            return _data_fix_to_string<int32_t, uint32_t, uint32_t>(
                    (uint32_t *)&val[0], "%d", "%u", "%*lf");
        case SSA_DATALENGTH_8BYTEFIX:
            return _data_fix_to_string<int64_t, uint64_t, uint64_t>(
                    (uint64_t *)&val[0], "%lld", "%llu", "%*lf");
...
void service_id::string_to_data(string buf, std::vector<uint16_t>& value) {
    py_value *pval = eval_full(buf);
    py_list *pval_list = dynamic_cast<py_list *>(pval);
    
    switch (data.attr.datalength) {
        case SSA_DATALENGTH_2BYTEFIX:
            value = _string_to_data_fix<int16_t, uint16_t, uint16_t, py_value>(pval);
            break;
        case SSA_DATALENGTH_4BYTEFIX:
            value = _string_to_data_fix<int32_t, uint32_t, float, py_value>(pval);
            break;
        case SSA_DATALENGTH_8BYTEFIX:
            value = _string_to_data_fix<int64_t, uint64_t, double, py_value>(pval);
            break;
        case SSA_DATALENGTH_1BYTEVAR:
            if (data.attr.datatype == SSA_DATATYPE_CHARSET) {
                ...
                if (pval_list) {
                    value = _string_to_data_var<int8_t, uint8_t, uint8_t, py_value>(pval_list->value);
                    break;
                }
            }
            break;
        case SSA_DATALENGTH_2BYTEVAR:
            if (pval_list) {
                value = _string_to_data_var<int16_t, uint16_t, uint16_t, py_value>(pval_list->value);
                break;
            }
        case SSA_DATALENGTH_4BYTEVAR:
            if (pval_list) {
                value = _string_to_data_var<int32_t, uint32_t, float, py_value>(pval_list->value);
                break;
            }
            break;
        case SSA_DATALENGTH_8BYTEVAR:
            if (pval_list) {
                value = _string_to_data_var<int64_t, uint64_t, double, py_value>(pval_list->value);
                break;
            }
            break;
        default:
